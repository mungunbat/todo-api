...
class Todo extends Model
{
    ...
    public function createTodo(array $attributes){
        $todo = new self();
        $todo->deadline = $attributes["deadline"];
        $todo->title = $attributes["title"];
        $todo->content = $attributes["content"];
        $todo->pic = $attributes["pic"];
        $todo->create_user_id = $attributes["create_user_id"];
        $todo->publish_type = $attributes["publish_type"];
        $todo->status = $attributes["status"];
        $todo->save();
        return $todo;
    }
